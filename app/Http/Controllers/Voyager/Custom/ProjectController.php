...
class ProjectController extends VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        // return Str::slug(json_decode($request->ourImpacts)[0]->title).'.'.$request->ourImpactIconImage[0]->getClientOriginalExtension();
        $impactIndex = 0;
        $ourImpactIcons = $request->ourImpactIconImage;

        // initializing object with empty array.
        $updatedImpactIcon = [];
        if(count(json_decode($request->ourImpacts))>0){
            foreach(json_decode($request->ourImpacts) as $item){

                    // getting icon one by one
                $impactIcon =  $ourImpactIcons[$impactIndex];

                    // initializing impactIconFinalLocation for line 399~
                $impactIconFinalLocation = null;
                ...
                if($impactIcon){
                    ...
                    if($impactIconFinalLocation){
                        if (file_exists(public_path('/storage/' . $impactIconFinalLocation))) {
                    ...
                    $impactIconLocation = 'projects/'.$projectSlug.'/our-impacts/icons/';
                    $impactIconName = $impactIconSlug.'.'.$impactIcon->getClientOriginalExtension();
                    $impactIcon->storeAs('public/'.$impactIconLocation,$impactIconName);
                    $impactIconFinalLocation = $impactIconLocation.$impactIconName;
                }

                // making same type of object for vue js
                $updatedImpactIcon[$impactIndex]['title'] = $item->title;
                $updatedImpactIcon[$impactIndex]['totalCount'] = $item->totalCount;
                $updatedImpactIcon[$impactIndex]['subScript'] = $item->subScript;
                if($impactIconFinalLocation){
                    $updatedImpactIcon[$impactIndex]['iconLocation'] = $impactIconFinalLocation;
                }
                else{
                    $updatedImpactIcon[$impactIndex]['iconLocation'] = '';
                }
        ...
        // initializing imageLocation as null
        $imageLocation = null;
        // check if coverImage is uploaded or not
        if ($request->coverImage) {
            $location = 'projects/'.$projectSlug.'/';
            $coverImageName = 'cover.'.$request->file('coverImage')->getClientOriginalExtension();
            $path = $request->file('coverImage')->storeAs('public/'.$location,$coverImageName);

            $imageLocation = $location.$coverImageName;
        }
        ...
        if ($request->menuImage) {
            $location = 'projects/' . $projectSlug . '/';
            $menuImageName = 'menu-image.' . $request->file('menuImage')->getClientOriginalExtension();
            $path = $request->file('menuImage')->storeAs('public/' . $location, $menuImageName);

            $menuImageLocation = $location . $menuImageName;
        }

        if($request->method=='post'){
            $project = new Project();
            $project->title = $request->title;
            $project->coverImage = $imageLocation;
            $project->short_info = $request->shortInformation;
            $project->facts = $request->facts;
            $project->description = $request->description;
            $project->faqs = $request->faqs;
            $project->sdgs = $request->sdgs;
            $project->slug =$projectSlug;
            $project->slider_id = $request->slider_id;
            $project->testimonial = $request->testimonial;
            $project->spotlight = $request->spotlight;
            $project->sub_projects = $request->subProjects;
            ...
            // cover Image Location upload
            if($imageLocation!=null){
                $project->coverImage = $imageLocation;
            }

            // menu Image Location upload
            if ($menuImageLocation != null){
                $project->menuImage = $menuImageLocation;
            }
